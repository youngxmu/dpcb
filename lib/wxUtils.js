...
var wxConfig = {
    qk : {
        appId : 'wxa72590692b5a8a3a',
        appSecret : 'f828b4facf2d2ad919c23677d379a340',
        noncestr : 'xkXGEs8VMCP',
        timestamp : '1414587499'
    },
    dpcb : {
        appId : 'wx6c194f81cd892341',
        appSecret : '00f78e54d9a3210184fb7dd5ed7a498c',
        noncestr : 'xkXGEs8VMCP',
        timestamp : '1414587499'
    },
...
var getTicket = function(type, callback){
    ...
    redisUtils.get(type + '_ticket',function(err, reply){
        ...
        if(ticket){
            console.log('get '  + type + ' ticket from redis:' + ticket);
...
module.exports = {
    ...
    getBaseInfoByOpenid : function(type, openid, callback){//getUserInfoByOpenid
        getToken(type, function(err, token){
            if(err){
                logger.warn('getBaseInfoByOpenid err ' + openid);
                console.log(err);
                callback(err);
                return;
            }
            ...
            rp(options).then(function (repos) {
                ...
                if(repos.errcode){
                    console.log(repos);
                    if(repos.errcode == 40001){
                        client.set(type + '_token', '', function(){
                            callback(repos);        
                        });
